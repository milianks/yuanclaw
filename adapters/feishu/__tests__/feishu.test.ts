...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/yuanclaw',
      realPath: '/Users/dev/yuanclaw',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**yuanclaw**')
    ...
    expect(info[1].content).toContain('yuanclaw')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/yuanclaw')
    expect(btn1.value.projectName).toBe('yuanclaw')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/yuanclaw')
    expect(event.action.value.projectName).toBe('yuanclaw')
  })
