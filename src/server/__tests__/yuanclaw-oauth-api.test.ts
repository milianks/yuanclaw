/**
 * Integration tests for /api/yuanclaw-oauth/* endpoints.
 */
...
async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'yuanclaw-oauth-api-test-'))
...
describe('POST /api/yuanclaw-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const { req, url, segments } = buildReq('POST', '/api/yuanclaw-oauth/start', {
      serverPort: 54321,
    })
    const res = await handleYuanclawOAuthApi(req, url, segments)
...
describe('GET /api/yuanclaw-oauth/status', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/yuanclaw-oauth/status')
    const res = await handleYuanclawOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    yuanclawOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/yuanclaw-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/yuanclaw-oauth')
    const res = await handleYuanclawOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await yuanclawOAuthService.loadTokens()).toBeNull()
  })
})
