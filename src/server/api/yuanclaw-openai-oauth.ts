/**
 * Yuanclaw OpenAI OAuth REST API
 *
 * POST   /api/yuanclaw-openai-oauth/start    — 生成 PKCE+state,返回 authorize URL
 * GET    /callback/openai                — 用户浏览器 redirect 到此,完成 token 交换
 * GET    /api/yuanclaw-openai-oauth          — 查询当前登录状态(不回传 token 本体)
 * DELETE /api/yuanclaw-openai-oauth          — 登出,删除 token 文件
 */
...
export async function handleYuanclawOpenAIOAuthApi(
  req: Request,
  url: URL,
  segments: string[],
): Promise<Response> {
  try {
    const action = segments[2] // segments: ['api', 'yuanclaw-openai-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = yuanclawOpenAIOAuthService.startSession({
    ...
    if (action === undefined && req.method === 'GET') {
      const tokens = await yuanclawOpenAIOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await yuanclawOpenAIOAuthService.deleteTokens()
...
export async function handleYuanclawOpenAIOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await yuanclawOpenAIOAuthService.completeSession(code, state)
...
</head><body><div class="card"><h1>✓ OpenAI Login Successful</h1><p>You can close this window and return to yuanclaw.</p></div>
